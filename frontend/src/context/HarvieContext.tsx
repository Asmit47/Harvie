...
import { AgentStatus, FloatingCardItem, ModalType, MemoryEntry, Integration } from '../types/harvie';
import { api } from '../lib/api';

const SESSION_STORAGE_KEY = 'harvie-session-id';

interface HarvieContextType {
...
const HarvieContext = createContext<HarvieContextType | undefined>(undefined);

export const HarvieProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    initializeSession().catch((err) => {
      console.warn('Could not initialize Harvie session:', err);
    });
      ...
      setRightZoneItems([
        {
          ...
          time: 'Due Today',
          context: 'Push Harvie spatial layout components to GitHub staging repo.',
          badge: 'Action Item',
          priority: 'high',
          zone: 'right',
          timestamp: '15:10',
  ...
  const processUserCommand = async (command: string) => {
    ...
    try {
      ...
      console.error('Error sending query to Harvie backend:', err);
      setAgentStatus('idle');
      setEphemeralMessage(`Harvie Backend: Failed to process query (${err.message || 'Server connection error'}). Make sure FastAPI server is running on port 8000.`);
    }
  };

  return (
    <HarvieContext.Provider
      value={{
        agentStatus,
        setAgentStatus,
        leftZoneItems,
...
export const useHarvie = () => {
  const context = useContext(HarvieContext);
  if (!context) {
    throw new Error('useHarvie must be used within a HarvieProvider');
  }
  return context;
};
